...
-----------------------------------------------------------
-- UI (second UI framework only)
-----------------------------------------------------------
do
    local base = UIBase.new():setLabel("UconfigE")
    base:setKeybind(Enum.KeyCode.RightShift)
    ...
    local rage = main:newSection("left", "Ragebot")
    rage:newToggle("uconfig/rage/enabled")
        :setLabel("Ragebot")
        .changed:Connect(function(v)
            setRage(v)
            Config.RageBotEnabled = v
            if Config.AutoSaveConfig then SaveConfig(Config.AutoSaveConfig, Config) end
        end)

    local equip = rage:newDropdown("uconfig/rage/auto_equip", false, {"gun", "bojo gun", "melee"})
        :set("gun")
        :setLabel("Auto Equip")
    equip.changed:Connect(function(v)
        AutoEquipCategory = v
        Config.AutoEquipCategory = v
        if Config.AutoSaveConfig then SaveConfig(Config.AutoSaveConfig, Config) end
    end)

    local weapon = main:newSection("right", "Weapon")
    weapon:newToggle("uconfig/weapon/no_recoil")
        :set(true)
        :setLabel("No Recoil")
        .changed:Connect(function(v)
            setNoRecoil(v)
    ...
    weapon:newToggle("uconfig/weapon/no_spread")
        :set(true)
        :setLabel("No Spread")
        .changed:Connect(function(v)
    ...
    local void = main:newSection("left", "Void Spam")
    void:newToggle("uconfig/void/enabled")
        :setLabel("Void Spam")
        .changed:Connect(function(v)
            setVoidSpam(v)
    ...
    local emote = main:newSection("right", "Emote")
    emote:newToggle("uconfig/emote/enabled")
        :setLabel("Emote Glitch")
        .changed:Connect(function(v)
            if v then
                EmoteEnabled = true
    ...
    -- ESP
    -------------------------------------------------------
    local espTab = tabList:newTab("ESP"):intoSections()

    local esp = espTab:newSection("left", "ESP")
    esp:newToggle("uconfig/esp/tracers")
        :setLabel("Tracers")
        .changed:Connect(function(v)
            ESPEnabled = v
            Config.ESPEnabled = v
    ...
    -- Movement
    -------------------------------------------------------
    local movement = tabList:newTab("Movement"):intoSections()

    local speed = movement:newSection("left", "Speed")
    speed:newSlider("uconfig/movement/walkspeed", 16, 200, 1)
        :set(16)
        :setLabel("Walk Speed")
        .changed:Connect(function(v)
            applyWalkSpeed(v)
    ...
    speed:newSlider("uconfig/movement/jumppower", 50, 300, 1)
        :set(50)
        :setLabel("Jump Power")
        .changed:Connect(function(v)
            applyJumpPower(v)
    ...
    local move = movement:newSection("right", "Movement")
    move:newToggle("uconfig/movement/infinitejump")
        :setLabel("Infinite Jump")
        .changed:Connect(function(v)
    ...
    move:newToggle("uconfig/movement/noclip")
        :setLabel("No Clip")
        .changed:Connect(function(v)
            setNoClip(v)
            Config.NoClip = v
    ...
    -- Config
    -------------------------------------------------------
    local configs = tabList:newTab("Config"):intoSections()

    local auto = configs:newSection("left", "Auto Settings")
    auto:newToggle("uconfig/config/autoload")
        :setLabel("Auto Load")
        .changed:Connect(function(v)
            Config.AutoLoad = v
            if Config.AutoSaveConfig then SaveConfig(Config.AutoSaveConfig, Config) end
    ...
    auto:newDropdown("uconfig/config/autoloadname", false, allConfigs)
        :set(Config.AutoLoadConfig or "default")
        :setLabel("Auto Load Config")
        .changed:Connect(function(v)
            Config.AutoLoadConfig = v
            if Config.AutoSaveConfig then SaveConfig(Config.AutoSaveConfig, Config) end
        end)

    auto:newDropdown("uconfig/config/autosavename", false, allConfigs)
        :set(Config.AutoSaveConfig or "default")
        :setLabel("Auto Save Config")
        .changed:Connect(function(v)
            Config.AutoSaveConfig = v
            if Config.AutoSaveConfig then SaveConfig(Config.AutoSaveConfig, Config) end
        end)

    local info = configs:newSection("right", "Config Info")
    info:newLabel("uconfig/config/info1")
        :setLabel("Configs are stored in UconfigE_Configs")
    info:newLabel("uconfig/config/info2")
        :setLabel("Use the selected config for auto save/load")
    ...
    menu:newLabel("uconfig/settings/keybind")
        :setLabel("Menu Key: RightShift")

    local status = settings:newSection("right", "Status")
    status:newLabel("uconfig/settings/status")
        :setLabel("UconfigE loaded")
    status:newLabel("uconfig/settings/ui")
        :setLabel("Second UI framework active")
